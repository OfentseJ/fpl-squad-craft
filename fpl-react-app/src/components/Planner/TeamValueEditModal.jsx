...
import { X, Save, TrendingUp, RotateCcw } from "lucide-react";

export default function TeamValueEditModal({
  isOpen,
  onClose,
  currentValue,
  onSave,
  ...
  useEffect(() => {
    if (isOpen) {
      setValue((currentValue / 10).toFixed(1));
    }
  }, [isOpen, currentValue]);
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-sm border border-gray-200 dark:border-gray-700 overflow-hidden transform transition-all scale-100">
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-3">
            Manually override the calculated team value.
          </p>
          ...
          <div className="mt-6 flex gap-3">
            {isManual && (
              <button
                type="button"
                onClick={handleReset}
                className="px-4 py-2.5 rounded-xl font-semibold text-red-600 bg-red-50 hover:bg-red-100 dark:bg-red-900/20 dark:hover:bg-red-900/40 transition-colors flex items-center justify-center"
